...
def opt_fn_with_sgd(
    ...
    for step in range(n_iters):
        x,y = xy
        loss = pathological_curve_loss(x,y)
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
...
class SGD:
    ...
    def step(self) -> None:
        ...
        not_first = len(self.b) > 1
        for param,bb in zip(self.params,self.b):
            gt = param.grad
            if self.lmda:
                gt += self.lmda*param
            if self.mu:
...
class RMSprop:
    ...
    @t.inference_mode()
    def step(self) -> None:
        for param,bb,vv in zip(self.params,self.b,self.v):
            gt = param.grad
            if self.lmda:
                gt += self.lmda*param
            vv *= self.alpha
            vv += (1-self.alpha)*gt**2
            vt = vv
            gt *= 1/(vt**0.5+self.eps)
            if self.mu:
                bb *= self.mu
                bb += gt
                gt = bb
...
class Adam:
    ...
    def step(self) -> None:
        for param,m,v in zip(self.params,self.m,self.v):
            ...
            param -= self.lr*mt/(t.sqrt(vt)+self.eps)
        self.t += 1
...
optimizer_list = [
    ...
    (Adam, {"lr": 0.15, "betas": (0.99, 0.99), "weight_decay": 0.5}),
    (AdamW, {"lr": 0.15, "betas": (0.99, 0.99), "weight_decay": 0.5}),
]
...
def update_args(args: WandbResNetFinetuningArgs, sampled_parameters: dict) -> WandbResNetFinetuningArgs:
    ...
    args.weight_decay = sampled_parameters['weight_decay'] if sampled_parameters['wd_on'] else 0.0
    return args
