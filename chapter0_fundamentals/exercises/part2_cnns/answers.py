...
class Linear(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        sqrt = np.sqrt(in_features)
        weight = (1/sqrt)*(2*t.rand(out_features,in_features)-1)
        self.weight = nn.Parameter(weight)
        bbias = (1/sqrt)*(2*t.rand(out_features)-1)
        self.bias = nn.Parameter(bbias) if bias else None
...
class SimpleMLP(nn.Module):
    def __init__(self):
        super().__init__()
        # for the Module to inherit the parameters from the submodules
        # they have to be defined in the __init__ method
...
def train(args: SimpleMLPTrainingArgs) -> tuple[list[float], list[float], SimpleMLP]:
    ...
    mnist_valloader = DataLoader(mnist_testset,batch_size=args.batch_size,shuffle=False)
...
class Conv2d(nn.Module):
    def __init__(
        ...
        sf = 1/np.sqrt(self.in_channels*self.kernel_size**2)
        self.weight  = nn.Parameter(sf*(2*t.rand(self.out_channels,self.in_channels,
...
class Sequential(nn.Module):
    ...
    def __getitem__(self, index) -> nn.Module:
        index %= len(self._modules)  # deal with negative indices
        index = str(index)
        return self._modules[index]
    def __setitem__(self, index, module: nn.Module) -> None:
        index %= len(self._modules)  # deal with negative indices
        index  = str(index)
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Use this data's mean & variance to normalize, then use it to update the buffers
            data_mean = t.mean(x,dim=(0,2,3),keepdim=False)
            data_var  = t.var(x,dim=(0,2,3),keepdim=False,unbiased=False)
            self.running_mean = ((1-self.momentum) * self.running_mean 
                                 + (self.momentum) * data_mean)
            self.running_var  = ((1-self.momentum) * self.running_var
                                 + (self.momentum) * data_var)
            self.num_batches_tracked += 1
        else:
            data_mean = self.running_mean
            data_var  = self.running_var
        data_mean = einops.repeat(data_mean, "feat -> 1 feat 1 1")
        data_var  = einops.repeat(data_var , "feat -> 1 feat 1 1")
        x = (x-data_mean)/t.sqrt(data_var+self.eps)
        weight = einops.repeat(self.weight, "feat -> 1 feat 1 1")
        bias   = einops.repeat(self.bias  , "feat -> 1 feat 1 1")
        x = x*weight + bias
        return x
...
class ResidualBlock(nn.Module):
    def __init__(self, in_feats: int, out_feats: int, first_stride=1):
        ...
        self.seq2 = Sequential(
            nn.Identity() if is_shape_preserving
            else Sequential(
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        return self.relu(self.seq1(x)+self.seq2(x))
...
class ResNet34(nn.Module):
    def __init__(
        ...
        super().__init__()
        out_feats0 = 64
        in_feats_per_group = [out_feats0,*out_features_per_group[:3]]
        self.n_blocks_per_group = n_blocks_per_group
        self.in_features_per_group = in_feats_per_group 
        self.out_features_per_group = out_features_per_group
        self.first_strides_per_group = first_strides_per_group
        self.n_classes = n_classes

        self.seq = Sequential(
            Conv2d(in_channels=3,out_channels=out_feats0,kernel_size=7
                   ,stride=2,padding=3),
            BatchNorm2d(num_features=out_feats0),
            ReLU(),
            MaxPool2d(kernel_size=3,stride=2),
            *[BlockGroup(n_blocks=n_blocks_per_group[i],in_feats=in_feats_per_group[i]
                         ,out_feats=out_features_per_group[i]
                         ,first_stride=first_strides_per_group[i]) for i in range(4)],
            AveragePool(),
...
def predict(
    ...
    """
    model.eval()
    probs = t.softmax(model(images),dim=-1)
...
def get_resnet_for_feature_extraction(n_classes: int) -> ResNet34:
    ...
    my_resnet.seq[-1] = Linear(my_resnet.out_features_per_group[-1],n_classes)
    return my_resnet    
...
def train(args: ResNetTrainingArgs) -> tuple[list[float], list[float], ResNet34]:
    ...
    cifar_valloader = DataLoader(cifar_testset,batch_size=args.batch_size,shuffle=False)

    optimizer = t.optim.Adam(model.seq[-1].parameters(), lr=args.learning_rate)
    loss_list = []
    accuracy_list = []
    ...
    for epoch in range(args.epochs):
        ...
        with t.inference_mode():
            ...
            for imgs,labels in cifar_valloader:
                ...
                guess  = t.argmax(logits,dim=1)
                correct += (guess == labels).sum().item()
            accuracy_list.append(correct/len(cifar_testset))
    return loss_list, accuracy_list, model
